...
# san -- train at 32x32.
#
# Workstation:  bash sh/train_32.sh                (detaches, prints the log path)
#               FOREGROUND=1 bash sh/train_32.sh   (stays attached; output still goes to the log)
...
# single launch with an env var (DATA=<zip> GPUS=<n> bash sh/train_32.sh). Anything
...
GPUS="${GPUS:-2}"
BATCH_GPU="${BATCH_GPU:-96}"          # per GPU
