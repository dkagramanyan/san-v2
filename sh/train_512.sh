...
#
# Workstation:  bash sh/train_512.sh                (detaches, prints the log path)
#               FOREGROUND=1 bash sh/train_512.sh   (stays attached; output still goes to the log)
...
# single launch with an env var (DATA=<zip> GPUS=<n> bash sh/train_512.sh). Anything
...
GPUS="${GPUS:-2}"
BATCH_GPU="${BATCH_GPU:-25}"          # per GPU
