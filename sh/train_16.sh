...
# san -- train at 16x16.
#
# Workstation:  bash sh/train_16.sh    (DATA=<zip> GPUS=<n> ... to override)
...
san-train \
    ...
    --gpus "${GPUS:-2}" \
    --batch-gpu "${BATCH_GPU:-320}" \
    --cond True --syn-layers "${SYN_LAYERS:-6}" \
    --precision "${PRECISION:-fp16}" \
    --kimg "${KIMG:-20000}" --tick "${TICK:-4}" --snap "${SNAP:-500}" --snapshot-keep-last "${KEEP_LAST:-1}" \
