...
# san -- train at 16x16.
#
# Workstation:  bash sh/train_16.sh                (detaches, prints the log path)
#               FOREGROUND=1 bash sh/train_16.sh   (stays attached; output still goes to the log)
# SLURM:        sbatch --account=<proj> --partition=<part> --nodes=1 --gpus=2 --cpus-per-task=8 --time=3-0:0 sh/train_16.sh
#
...
# single launch with an env var (DATA=<zip> GPUS=<n> bash sh/train_16.sh). Anything
...
OUTDIR="${OUTDIR:-./runs}"
CFG="${CFG:-stylegan3-r}"
DATA="${DATA:-./datasets/imagenet_9to4_1024x1024_16x16.zip}"
GPUS="${GPUS:-2}"
BATCH_GPU="${BATCH_GPU:-320}"         # per GPU
SYN_LAYERS="${SYN_LAYERS:-6}"
PRECISION="${PRECISION:-fp16}"
KIMG="${KIMG:-20000}"
TICK="${TICK:-4}"                     # kimg per tick
SNAP="${SNAP:-500}"                   # ticks per snapshot + combra eval
